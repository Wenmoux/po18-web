...
class NovelCrawler {
    ...
    async search(keyword, page = 1) {
        try {
            ...
            // 获取分页信息
            const totalPages = $('.pagination').find('li').length > 0 
    ...
    async getDetail(bookId) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                // 获取标签 - 多种选择器尝试
                const tags = [];
                $('.book_intro_tags a, .book_intro_tags > a').each((i, el) => {
                ...
                // 获取描述 - 多种选择器尝试
                const description = $('.B_I_content').text().trim() || 
                                   $('.book_intro_content').text().trim() || 
                                   '';
                
                // 获取标题 - 多种选择器尝试
                const fullTitle = $('h1.book_name').text().trim() || 
                                 $('h1').first().text().trim() || 
                                 '';
                const title = fullTitle ? fullTitle.split(/（|【|\(/)[0].trim() : '';
                
                // 获取作者 - 多种选择器尝试
                const author = $('a.book_author').text().trim() || 
                              $('.book_author').text().trim() || 
                              '';
                
                // 获取封面 - 多种选择器尝试
                const cover = $('.book_cover img').attr('src') || 
                             $('img.book_cover').attr('src') || 
                             '';
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    console.log(`章节 [${bookId}/${chapterId}] 下载失败 (${error.message})，${delay}ms后重试 ${attempt + 1}/${maxRetries}`);
                ...
                try {
                    ...
                    if (!content) {
                        ...
                        if (ChapterCacheDB && !content.error) {
                            try {
                                ChapterCacheDB.save(chapter.bookId, chapter.chapterId, content.title || '', content.html || '', content.text || '');
                            } catch (err) {
    ...
    async getPurchasedBooks(years = 5) {
        ...
        for (let year = currentYear; year >= currentYear - years; year--) {
            ...
            while (retryCount <= maxRetries) {
                try {
                    console.log(`正在获取${year}年已购书籍...${retryCount > 0 ? ` (重试 ${retryCount}/${maxRetries})` : ''}`);
...
<body>
    <div class="book-header">
        ...
        <div class="book-tags">
            ${detail.tags.split('·').map(t => t.trim() ? `<span>${t.trim()}</span>` : '').join('')}
        </div>
    </div>
    ...
    ${contents.map((chapter, i) => chapter ? `
    ...
    toEpubData(detail, contents) {
        return {
            ...
            publisher: 'PO18小说下载站',
            cover: detail.cover,
            content: contents.filter(c => c).map((chapter, i) => ({
